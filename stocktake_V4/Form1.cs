...
namespace stocktake_V4
{
    public partial class Form1 : MetroForm
    {
        ...
        public Form1(int idUser, String userName, int level, int type_scan)
        {
            try
            {
                ...
                if (!String.IsNullOrEmpty(txtQty.Text))
                {
                    if (!String.IsNullOrEmpty(txtScanedSerial.Text))
                    {
                        ...
                        if (zone != "NOT_FOUND")
                        {
                            ...
                            String template = m_db_st.getLabelST(model, djGroup, semifinish, cgsRoute, qty, qr, mag, printer,type_cont).First().label;

                            if (template != "DOESNT_EXIST")
                            {
                                if (m_db_st.insertLabel(idU, serial, model, djGroup, semifinish, cgsRoute, qty, qr, mag,type_cont).First().RESULT != -1)
                                {
